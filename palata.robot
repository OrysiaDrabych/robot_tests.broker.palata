...
Підготувати клієнт для користувача
  ...
  Set Global Variable   ${PALATA_LOT_ID}   ${EMPTY}
  Set Global Variable    ${PALATA_MODIFICATION_DATE}   ${EMPTY}
  ...
  Run Keyword If  '${username}' != 'palata_Viewer'  Login  ${username}
...
Створити тендер
  ...
  ${guarantee}=           Convert To String   ${guarantee}
   ...
   Set Global Variable   ${PALATA_LOT_ID}   ${lotID}
   ...
   Click Link   xpath=//a[contains(@href,'${PALATA_LOT_ID}')]
...
Пошук тендера по ідентифікатору
    ...
    ${flag}=  Run Keyword And Return Status  Should Be Empty  ${PALATA_LOT_ID}
...
Пошук тендера у разі наявності змін
  ...
  ${status}=   Run Keyword And Return Status   Should Not Be Equal   ${PALATA_MODIFICATION_DATE}   ${last_mod_date}
  Run Keyword If   ${status}   palata.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Set Global Variable   ${PALATA_MODIFICATION_DATE}   ${last_mod_date}
...
Отримати кількість предметів в тендері
    ...
    palata.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
...
Чи фінасова процедура
  ...
  ${procedureType}=   convert_palata_string_to_common_string   ${procedureType}
...
Подати цінову пропозицію
  ...
  palata.Пошук тендера по ідентифікатору   ${username}   ${auction_id}
...
Завантажити документ в ставку
  ...
  ...      ${ARGUMENTS[1]} ==  ${filepath}
  ...      ${ARGUMENTS[2]} ==  ${TENDER_UAID}
  palata.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[2]}
...
Перевірити та сховати повідомлення
  ${status}=   Run Keyword And Return Status   Element Should Be Visible   id=close_inform_window
  Run Keyword And Return If   '${status}' == 'True'   Сховати повідомлення
  Reload Page
...
Отримати інформацію із тендера
  ...
  Run Keyword If    "${resp}" == "False"   palata.Пошук тендера по ідентифікатору   ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію про status
  ...
  ${status}=   convert_palata_string_to_common_string   ${return_value}
  [return]   ${status}
...
Отримати інформацію про procurementMethodType
  ...
  ${return_value}=   convert_palata_string_to_common_string   ${return_value}
  [return]   ${return_value}
...
Отримати інформацію із запитання
  ...
  palata.Пошук тендера у разі наявності змін   ${TENDER['LAST_MODIFICATION_DATE']}  ${ARGUMENTS[0]}   ${ARGUMENTS[1]}
...
Отримати інформацію із документа по індексу
  ...
  ${text}=  convert_palata_string_to_common_string   ${text}
  [return]  ${text}
...
Отримати кількість документів в ставці
  ...
  palata.Пошук тендера по ідентифікатору   ${username}   ${tender_uaid}
  Зайти в розділ кваліфікація
  ${drop_id}=  Catenate   SEPARATOR=   ${PALATA_LOT_ID}   _pending
...
Отримати дані із документу пропозиції
  ...
  ${doc_value}=   convert_palata_string_to_common_string   ${doc_value}
  [return]  ${doc_value}
...
Дискваліфікувати постачальника
  ...
  Зайти в розділ кваліфікація
  ${verification}=  Catenate   SEPARATOR=   ${PALATA_LOT_ID}   _pending.verification
  ${payment}=  Catenate   SEPARATOR=   ${PALATA_LOT_ID}   _pending.payment
  ${active}=  Catenate   SEPARATOR=   ${PALATA_LOT_ID}   _active
  Wait Until Keyword Succeeds   10 x   20 s   Run Keywords
  ...   Reload Page
...
Завантажити протокол аукціону в авард
   ...
   Зайти в розділ кваліфікація
   ${drop_id}=  Catenate   SEPARATOR=   ${PALATA_LOT_ID}   _pending.verification
...
Завантажити рішення ФГВФО
  [Arguments]   ${username}  ${tender_uaid}
  ${file_path}  ${file_name}  ${file_content}=  create_fake_doc
  palata.Завантажити документ в тендер з типом   ${username}  ${tender_uaid}  ${file_path}  clarifications

Внести зміни в title
   [Arguments]   ${value}
   Input Text    id=title   ${value}

Внести зміни в description
   [Arguments]   ${value}
   Input Text    id=desc   ${value}

Внести зміни в dgfID
   [Arguments]   ${value}
   Input Text    id=OpLotForm_op_dgfID   ${value}

Внести зміни в dgfDecisionDate
   [Arguments]   ${value}
   Input Text    id=OpLotForm_op_dgfDecisionDate   ${value}

Внести зміни в dgfDecisionID
   [Arguments]   ${value}
   Input Text    id=OpLotForm_op_dgfDecisionID   ${value}

Внести зміни в tenderAttempts
   [Arguments]   ${value}
   ${value}=   Convert To String    ${value}
   Select From List By Value   xpath=//select[@id='OpLotForm_number_auction']  ${value}

Внести зміни в тендер
   [Arguments]   ${user_name}   ${tender_uaid}   ${tender_field}   ${field_value}
   Run Keyword If   '${tender_field}'=='title'   Завантажити рішення ФГВФО   ${user_name}   ${tender_uaid}
   Зайти в розділ списку лотів
   ${drop_id}=  Catenate   SEPARATOR=   lot_  ${PALATA_LOT_ID}
   ...
   Wait Until Page Contains   Успішно відредаговано
   Перевірити та сховати повідомлення
